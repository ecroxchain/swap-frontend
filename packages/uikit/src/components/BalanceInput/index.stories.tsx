...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"SECROX" | "USD">("SECROX");
  const conversionUnit = editingUnit === "SECROX" ? "USD" : "SECROX";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "SECROX" ? "USD" : "SECROX";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "SECROX") {
      ...
      setValues({
        SECROX: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
