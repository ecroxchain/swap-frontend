...
import { Modal, Text, Flex, BalanceInput, Box, Button, LogoRoundIcon } from '@secrox/uikit'
...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.secrox.address)
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient SECROX balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% SECROX or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      <ExistingInfo>
        <Flex justifyContent="space-between">
          ...
          <Text>{t('%num% SECROX', { num: getBalanceNumber(amount).toLocaleString() })}</Text>
        </Flex>
        <Flex justifyContent="space-between">
      ...
      <InnerContent>
        <Flex justifyContent="space-between" alignItems="center" pb="8px">
          ...
          <Flex>
            ...
            <Text bold>SECROX</Text>
          </Flex>
        </Flex>
        {isFirstBid && (
          <Text pb="8px" small>
            {t('First bid must be %initialBidAmount% SECROX or more.', { initialBidAmount })}
          </Text>
        )}
        <BalanceInput
        ...
        <Text color="textSubtle" small mt="24px">
          {t('If your bid is unsuccessful, you’ll be able to reclaim your SECROX after the auction.')}
        </Text>
      </InnerContent>
    </StyledModal>
  )
