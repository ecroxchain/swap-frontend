...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCake = ifo.currency === tokens.secrox
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of SECROX up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max SECROX entry is capped by minimum between your average SECROX balance in the IFO SECROX pool, or the pool’s hard cap. To increase the max entry, Stake more SECROX into the IFO SECROX pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max SECROX entry is capped by your average SECROX balance in the IFO SECROX pool. To increase the max entry, Stake more SECROX into the IFO SECROX pool',
        )
      : t(
          'For the unlimited sale, Max SECROX entry is capped by your average SECROX balance in the IFO SECROX pool. To increase the max entry, Stake more SECROX into the IFO SECROX pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. SECROX entry') : t('Max. token entry')
...
const IfoCardDetails: React.FC<IfoCardDetailsProps> = ({ isEligible, poolId, ifo, publicIfoData, walletIfoData }) => {
  ...
  // unlimited pool just show the ecrox left
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('SECROX to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
