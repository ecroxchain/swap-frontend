...
import { Text, Flex, AccountIcon, TeamBattleIcon, Box, useTooltip, LinkExternal } from '@secrox/uikit'
...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%SECROX Squad NFT%'

  const rawText = t(`Set %SECROX Squad NFT% as SECROX Profile avatar`)
      ...
      element={
        <>
          <LinkExternal
            ...
            href={`http://ecroxswap.com/nfts/collections/${admissionProfile}`}
          >
            {t('SECROX Squad NFT')}
          </LinkExternal>
          <br />
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%SECROX Profile%'

  const rawText = t(`Reach %point% or more %SECROX Profile% points`, { point: pointThreshold })
    ...
    <TransWithElement
      ...
      element={
        <>
          ...
          <LinkExternal
            style={{ display: 'inline' }}
            href={`http://ecroxswap.com/nfts/profile/${account}/achievements/`}
          >
            {t('SECROX Profile')}
          </LinkExternal>
        </>
