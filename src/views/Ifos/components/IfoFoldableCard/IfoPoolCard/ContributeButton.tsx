...
import { Button, useModal } from '@secrox/uikit'
...
const ContributeButton: React.FC<Props> = ({ poolId, ifo, publicIfoData, walletIfoData }) => {
  ...
  const handleContributeSuccess = async (amount: BigNumber, txHash: string) => {
    ...
    toastSuccess(
      ...
      <ToastDescriptionWithTx txHash={txHash}>
        {t('You have contributed %amount% SECROX to this IFO!', {
  ...
  return (
    <Button
      ...
      disabled={isDisabled}
    >
      {isMaxCommitted && publicIfoData.status === 'live' ? t('Max. Committed') : t('Commit SECROX')}
    </Button>
  )
}
