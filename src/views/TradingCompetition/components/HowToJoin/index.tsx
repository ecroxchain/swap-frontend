    ...
    <Flex flexDirection="column" maxWidth="736px">
      ...
      <HowToCard number={1} title={t('Get Ready')}>
        <Text fontSize="14px" color="textSubtle">
          {t('Set up your')}{' '}
          {account ? (
            <StyledLink href={`${nftsBaseUrl}/profile/${account.toLowerCase()}`}>{t('SECROX Profile')}</StyledLink>
          ) : (
            t('SECROX Profile')
          )}
          {', '}
      ...
      <HowToCard number={2} title={t('Battle Time')}>
        <Text fontSize="14px" color="textSubtle">
          {t(
            'Trade DAR/ECROX, SECROX/ECROX, and SECROX/BUSD during the battle period to raise both your and your team’s score. See the Rules section below for more.',
          )}
        </Text>
      </HowToCard>
