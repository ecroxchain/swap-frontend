...
describe('Config pools', () => {
  ...
  it.each(pools.map((pool) => [pool.sousId, pool.contractAddress]))(
    ...
    (sousId, contractAddress) => {
      const duplicates = pools.filter((p) => contractAddress[988207] === p.contractAddress[988207])
      expect(duplicates).toHaveLength(1)
    },
  )
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'ECROX'))(
  ...
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'ECROX'))(
