import { FACTORY_ADDRESS } from '@danielvindax/ecroxswap-sdk'
  ...
  const usersQuery = gql`
    query userCount($since: ISO8601DateTime, $till: ISO8601DateTime) {
      ethereum(network: bsc) {
        dexTrades(exchangeName: { in: ["SECROX", "SECROX v2"] }, date: { since: $since, till: $till }) {
          count(uniq: senders)
        }
      }
    }
  ...
  try {
    const result = await infoServerClient.request(gql`
      query tvl {
        ...
        token(id: "0x6e32F26C48A1155bE3Ae90017320c6B615Bac3c8") {
          derivedUSD
        }
      }
    `)
