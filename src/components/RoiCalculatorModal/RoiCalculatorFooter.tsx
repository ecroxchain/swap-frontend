...
import { Flex, Box, Text, ExpandableLabel, LinkExternal, Grid, HelpIcon, useTooltip } from '@secrox/uikit'
...
const RoiCalculatorFooter: React.FC<RoiCalculatorFooterProps> = ({
  ...
  const {
    ...
    <>
      <Text>
        {t(
          'The Multiplier represents the proportion of SECROX rewards each farm receives, as a proportion of the SECROX produced each block.',
        )}
      </Text>
      <Text my="24px">
        {t('For example, if a 1x farm received 1 SECROX per block, a 40x farm would receive 40 SECROX per block.')}
      </Text>
      ...
      {isExpanded && (
        <Box px="8px">
          <Grid gridTemplateColumns="2.5fr 1fr" gridRowGap="8px" gridTemplateRows={`repeat(${gridRowCount}, auto)`}>
            ...
            {!Number.isFinite(apy) ? (
              <Text color="contrast" small>
                {isFarm ? t('Base APR (SECROX yield only)') : t('APR')}
              </Text>
            ) : (
              <Text color="contrast" small>
                {t('APY')}
